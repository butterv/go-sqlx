...
ENV GOPATH=/go \
    GO111MODULE=on \
    PROJECT_ROOT=/go/src/github.com/butterv/go-sqlx
...
RUN CGO_ENABLED=0 GOOS=linux go build -o server -a ./app/cmd/server

# image for release
FROM gcr.io/distroless/base:latest
ENV BUILDER_ROOT /go/src/github.com/butterv/go-sqlx
ENV PROJECT_ROOT /
